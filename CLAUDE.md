...
- **태그코드 포맷 확정(§6.3)**: `숙박prefix-팀알파-개인번호`(3코드, 예 `H-A-01`). 숙박 prefix는 **H(호텔)/C(카라반)/X(미정)**. 팀알파는 **출/귀국 사이클(현장 체류가 겹치는 팀 묶음) 단위**로 A부터 부여하고, 앞 사이클이 모두 귀국하면 다음 사이클에서 다시 A로 **리셋**(체류 겹침 없으면 코드 재사용). 구현은 `app/index.html`의 `buildTagCodeMap`(cohort 경계 = dep > 현재 사이클 최대 arr) / `makeTagCode`(`TAG_BASE`+개인번호). *(구 포맷 `H-0702-A-01`의 입국MMDD는 제거됨)*
...
3. ~~태그코드 포맷 확정~~ ✅ **확정**: `숙박prefix-팀알파-번호`(3코드, 예 `H-A-01`). 팀알파는 출/귀국 사이클 단위 리셋. §3 참조. *(구 4코드 `H-0702-A-01`에서 입국MMDD 제거)*
