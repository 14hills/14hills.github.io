...
- **태그코드 포맷 확정(§6.3)**: `요일+출발지-팀알파-개인번호`(예 `MI-A-01`). **1번째=입국 송영 요일**(14hills 송영은 火/木/日 고정 → 火=**K**·木=**M**·日=**N**), **2번째=출발지**(인천=**I**·부산=**P**·청주=**C**·대구=**T**), **3번째=팀알파**(A,B,C…), **4번째=개인번호**(01,02…). 팀알파는 **출/귀국 사이클(현장 체류가 겹치는 팀 묶음) 단위**로 prefix(`요일+출발지`)별 A부터 부여하고, 앞 사이클이 모두 귀국하면 **소멸→다음 사이클에서 다시 A로 리셋(코드 재사용)**. 구현은 `app/index.html`의 `dowTagCode()`·`originTagCode()` + `buildTagCodeMap`(cohort 경계 = dep > 현재 사이클 최대 arr, `TAG_BASE[seq]='{요일}{출발지}-{팀알파}'`) / `makeTagCode`(`TAG_BASE`+개인번호). *(구 포맷 `H-A-01`(숙박prefix) → 현 요일+출발지 포맷으로 교체. 숙박 H/C/X는 색상 구분에만 사용)*
...
3. ~~태그코드 포맷 확정~~ ✅ **확정**: `요일+출발지-팀알파-번호`(예 `MI-A-01`). 요일=火K/木M/日N, 출발지=인천I/부산P. 팀알파는 출/귀국 사이클 단위 리셋(소멸·재사용). §3 참조. *(구 포맷 `H-A-01`(숙박prefix) → 요일+출발지로 교체)*
...
## 6.5 UI 규칙 — 아이콘(이모지) 사용 금지 ✅ 확정
- **UI에 이모지/그림 아이콘을 쓰지 않는다**(🖨☁📅🗑📝🛫🛬🚌🏨🍽 등). 버튼·라벨·제목·토스트·placeholder 모두 **텍스트만** 사용한다. 아이콘만 있던 버튼은 텍스트 라벨로 교체(예: 🗑→「삭제」).
- 허용 기호(아이콘 아님): 닫기 `✕`, 체크 `✓`/`✗`, 경고 `⚠`, 펼침/화살표 `▶ ◀ ▲ → ← ↔ ↩`, 원문자 `① ② ③`, 구분선 `─ ═`. 이들은 텍스트 기호로 유지 가능.
- 새 기능 추가 시에도 이모지를 넣지 말 것.
